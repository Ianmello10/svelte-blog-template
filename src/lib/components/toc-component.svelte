...
{#if tocItems && tocItems.length > 0}
	<div
		...
		class="bg-base-200/50 border-base-content/10 w-full rounded-lg border shadow-sm backdrop-blur-sm"
	>
		<button
			class="hover:bg-base-300 flex w-full items-center justify-between p-4 text-left transition-colors duration-200"
		...
		>
			<div class="flex items-center gap-3">
				<div class="bg-primary/10 rounded-lg p-2">
					<BookOpen class="text-primary-content h-4 w-4" />
				</div>
				<div>
			...
			<div class="flex items-center gap-2">
				<span class="text-base-content/60 hidden text-xs sm:inline">
					{isExpanded ? 'Recolher' : 'Expandir'}
				</span>
				<ChevronDown
					class={`text-base-content/60 h-4 w-4 transition-transform duration-300${isExpanded ? ' rotate-180' : ''}`}
				/>
			</div>
		</button>
								...
								{#each tocItems as item, index (item.href)}
									...
									<li in:fly={{ x: -15, delay: index * 40, duration: 250, easing: quintOut }}>
										<button
											...
											class="group text-base-content/60 hover:bg-base-300 w-full rounded-md px-3 py-2 text-left text-sm transition-all duration-200"
												...
												<div class="mt-1.5 flex items-center">
													...
													<div
														class={{
															...
															'bg-base-content/40': item.depth - minDepth === 1,
															'bg-base-content/30': item.depth - minDepth >= 2
														}}
													/>
												</div>
